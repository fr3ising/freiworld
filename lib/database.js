...
function insertLink(uri,title,comment,nick) {
    pool.getConnection(function(err,connection) {
	if ( err ) {
	    if (connection) connection.release();
	    throw err;
	}
	console.log("HOLASKDJLAKSJD");
	connection.query("SELECT id FROM users WHERE nick=?;",function(err,rows) {
	    console.log(rows[0]);
	    connection.query("INSERT INTO links (user_id,uri,title,comment) VALUES (?,?,?,?);",
			     [rows[0],uri,title,comment],function(err,rows) {
				 if ( ! err ) {
				     console.log(rows);
				 } else {
				     console.log(err);
